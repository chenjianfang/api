...
//原生JS获取元素属性值
export function eleStyle (ele,prop) {
	var objStyle = ele.currentStyle ? ele.currentStyle : window.getComputedStyle(ele,null);
	if(objStyle.getPropertyValue){
		return objStyle.getPropertyValue(prop);
	}else{
		prop = prop.replace(/-([a-z]{1})/,function(){
			return arguments[1].toUpperCase();
		});
		return objStyle.getAttribute(prop);
	}
}
...
export const winRequestAnimationFrame = function (callback) {
	var st = window.requestAnimationFrame       ||
